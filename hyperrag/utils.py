...
def limit_async_func_call(max_size: int, waitting_time: float = 0.0001):
    """使用 asyncio.Semaphore 限制异步函数的最大并发调用数

    采用事件循环感知的延迟创建策略：每次调用时检查当前事件循环是否与
    Semaphore 绑定的循环一致，不一致则重建 Semaphore，
    避免因事件循环切换导致 "is bound to a different event loop" 错误。
    """
    ...
    def final_decro(func):
        ...
        async def wait_func(*args, **kwargs):
            ...
            if _sem[0] is None or _sem_loop[0] is not loop:
                _sem[0] = asyncio.Semaphore(max_size)
                _sem_loop[0] = loop
            async with _sem[0]:
